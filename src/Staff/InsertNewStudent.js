...
const Form = ({ closePopup }) => {
  ...
  const [batchOptions, setBatchOptions] = useState([]); // State for batch options

  const token = localStorage.getItem('authToken');

  useEffect(() => {
    // Fetch batch options from API on component mount
  ...
  const handleSubmit = async (e) => {
    ...
    const postData = {
      ...
      batch: formData.batchId, // Use batchId for submission
      fee: {
