...
import { School, Group, BatchPrediction, Notifications } from '@mui/icons-material';
    ...
    const fetchCounts = async () => {
      try {
        const [staffResponse, studentResponse, batchResponse, notificationResponse] = await Promise.all([
          ...
          axios.get('http://127.0.0.1:8000/studentportal/notification/')
        ]);
  ...
  return (
    <div className="container">
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card className="notifications" sx={{ background: 'linear-gradient(135deg, #F48FB1 0%, #F06292 100%)', color: '#fff', borderRadius: '15px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)' }}>
            <Notifications sx={{ fontSize: 40, color: '#fff' }} />
            <CardContent sx={{ padding: '24px', textAlign: 'center' }}>
              <Typography variant="h5" sx={{ mt: 1 }}>Notifications</Typography>
              <Typography variant="h6" sx={{ mt: 1 }}>{notificationCount}</Typography>
            </CardContent>
          </Card>
        </Grid>
      </Grid>
